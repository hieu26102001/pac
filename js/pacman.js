...
var pacman_mounth = 3;
var pacman_mounth_MAX = 6;
var PACMAN_SIZE = 16;
var PACMAN_MOVING = false;
var PACMAN_MOVING_TIMER = -1;
var PACMAN_MOVING_SPEED = 18;
...
function movePacman(direction) {
	...
	if ((!directionTry || PACMAN_DIRECTION !== directionTry)) { 
		...
		if (canMovePacman(PACMAN_DIRECTION)) { 
			...
			if (pacman_mounth < pacman_mounth_MAX) { 
				pacman_mounth ++; 
			} else { 
				pacman_mounth = 0; 
			}
			...
			if ((pacman_mounth) === 0 || (pacman_mounth) === 3) { 
				testBubblesPacman();
				testGhostsPacman();
				testFruitsPacman();
			}
...
function drawPacman() { 
	...
	if (PACMAN_DIRECTION === 1) { 
		startAngle = (0.35 - (pacman_mounth * 0.05)) * Math.PI;
		endAngle = (1.65 + (pacman_mounth * 0.05)) * Math.PI;
		lineToX -= 8;
	} else if (PACMAN_DIRECTION === 2) { 
		startAngle = (0.85 - (pacman_mounth * 0.05)) * Math.PI;
		endAngle = (0.15 + (pacman_mounth * 0.05)) * Math.PI;
		lineToY -= 8;
	} else if (PACMAN_DIRECTION === 3) { 
		startAngle = (1.35 - (pacman_mounth * 0.05)) * Math.PI;
		endAngle = (0.65 + (pacman_mounth * 0.05)) * Math.PI;
		lineToX += 8;
	} else if (PACMAN_DIRECTION === 4) { 
		startAngle = (1.85 - (pacman_mounth * 0.05)) * Math.PI;
		endAngle = (1.15 + (pacman_mounth * 0.05)) * Math.PI;
		lineToY += 8;
	}
